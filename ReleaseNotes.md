...
* vbv_calc,vbv_formationにゲーム画面からの情報取得ボタンの追加  
  ...
  vbv_calcではキャラクター、装備、称号を自動的にセットします。
...
* vbv_calcの保存リストにソート機能などを追加
  師団をゲーム画面から情報を取得してロードする際、同名キャラがいると上のほうが優先されるため全体のソートとリストを動かす機能を作りました。
  移動させた後にファイルに保存ボタンを押してください。多重書き込みの問題からvbv_formationからは変更できない仕様です。そのうちアプリを1つにしますので…

* 英雄覇気の修正
  報告から、活性＋支配＋指揮の3つの効果としています。
