# はじめに
ここではvbv_calcの内部的な仕様や注意事、使い方などいろいろ書いています。適当に読み飛ばしちゃってもVBVの画面に近くしたつもりなのでフィーリングで使えるはずです。
...
左側のリストでは設定したキャラクターデータを保存・読み込みできます。キャラクター名、装備、称号、糧食、リーダーフラグを保存します。一時的なものなので、再起動すると消えます。
...
###  職業について
職業による攻撃倍率の増加は考慮されていません
データは入っているはずなので、そのうち入れるかもしれません。

### レギオンスタンスについて
考慮していません。職業と同じくJSONからロードしていると思われるのでそのうち入れるかもしれません。
...
### 凍傷気流
種族は考慮されるが、血の起源は無視。種族を考慮といっても防御側が該当種族を持っていたら無視して計算をしない、という割りきりです。
どちらも上限30%として、値を相殺してから残ってたら計算。相殺後、30上限を行います。
上記はダメージ計算の場合で、倍率計算の時は必ず発揮されるとしています。

### 聖戦の導き
どちらも上限30%として、値を相殺してから残ってたら計算。スキル値は30以上あっても無視(上限以上のスキル値は意味がない)
上記はダメージ計算の場合で、倍率計算の時は必ず発揮されるとしています。
...
特攻とクリティカルの計算のみ、防御側の種族と速度ステータスを考慮。
狂奔などのステータスUP系はその倍率を攻撃倍率にかけている。ステータスUPも（基本は）積算関係という認識のもと、作られています。
...
さしあたって、data以下にあるjsonファイルのいらないデータを消して再起動するのが一番手っ取り早いです。
一応、セレクトボックスで単純な検索はできるはずですが、使い勝手がよくないのでそのうちうまい方法見つけて改良したいです。

* キャラクターの選択・フィルターが重い  

700ぐらいあるからか遅いです、改善したいです。
