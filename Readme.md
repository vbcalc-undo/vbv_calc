...
# 依存関係と動作環境
このソフトウェアでは様々なライブラリを使用しています。詳細はLicense.txtをご確認ください。
また、C#と.NETバージョンは以下です。.NET8.0のランタイムが必要ですので、なければ入れてください。
- .NET 8（ターゲットフレームワーク）
- C# 12.0（言語バージョン）

また、本ソフトウェアはWindows11 64bitでしか動作確認していません。LinuxやMacOSで動くかはわかりません。
以前まではWindows7でも動いていたはずですが、v1.0.7より明確にWindows10からしか動かないかと思います。Windows10も動くか確認していません。
...
また、キャプチャは最小化していると動作しないため、エラーが出るはずです。最前面で画面に写っている必要はありませんが、最小化はしないでください。

なお、類似画像検索については特徴量を抽出したものを同梱しており、元画像を配布しているわけではないです。
この特徴量は不可逆（特徴量から画像を再現することは不可能）なため、著作権的にも問題ないはずです。

キャラクター名は大体大丈夫だと思いますが、装備や称号は一部取り込みづらい文字があるので、チェックをお願いします。
グンヒルドの称号で杖の→槍のがあることは確認しました。なかなか調整が難しいので精度をこれ以上上げるかは不明です。
...
編成画面を開き、取り込みたい師団を表示させてください。編成画面では6師団表示されていますので、これを指定できるボックスをすぐ右に作っています。
...
英雄覇気については、wikiの方から活性と支配で2回かかっているというご指摘を受けました。
おそらく1.0.4の動作ではそれが正しそうなため、その計算式を使っています。ありがとうございました。
