...
左側のリストでは設定したキャラクターデータを保存・読み込みできます。キャラクター名、装備、称号、糧食、リーダーフラグを保存します。セレクトしている状態で削除ボタンを押すと削除できます。

ファイルに保存は、json/saved.jsonというJSONファイルに保存されます。ファイルから読み込みでこのJSONファイルを読み込みリストを再構築します。バックアップなどは手動で取得してください。
ファイルから読み込みボタンを押すと有無を言わさずリストを再構築しますので、保存していないデータは消えます。注意してください。
終了時に保存するという気の利いたことはしてくれないので、定期的に保存ボタンを押してください。
...
他と重複しそうですが、運命改変は攻撃側・防御側ともに、以下の要素を考慮しています。
